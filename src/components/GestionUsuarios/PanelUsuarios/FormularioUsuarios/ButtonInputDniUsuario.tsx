...
import { RegisterUsuario } from "./inputsUsuarios/inpustNameUsuario";

const ButtonInputDniUsuario = () => {
  const { values, setFieldValue } = useFormikContext<RegisterUsuario>();
  ...
  useEffect(() => {
    if (data) {
      const { apellidoMaterno, apellidoPaterno, nombres, numeroDocumento } =
        data;
      updateDatDni(data);
      if (
...
export default ButtonInputDniUsuario;
